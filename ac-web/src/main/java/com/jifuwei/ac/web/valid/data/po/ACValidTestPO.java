package com.jifuwei.ac.web.valid.data.po;
...
public class ACValidTestPO {
    ...
    private byte[] mysql_blob;          //二进制大对象:一般用于存储文件字符流等信息,String代表文件名
    ...
    private boolean mysql_boolean;      //布尔型：mysql默认转成tinyint(1),false为0，true为1
    ...
    private String mysql_year;            //年：YYYY
    ...
    public byte[] getMysql_blob() {
        return mysql_blob;
    }

    public void setMysql_blob(byte[] mysql_blob) {
    ...
    public boolean isMysql_boolean() {
        return mysql_boolean;
    }

    public void setMysql_boolean(boolean mysql_boolean) {
