package com.jifuwei.ac.foundation.dao;

import com.jifuwei.ac.foundation.constant.OperateConstant;
import com.jifuwei.ac.foundation.data.ACPageInfo;
...
public abstract class IDaoImpl<Data> implements IDao<Data> {
    ...
    public boolean update(Data d) {
        ...
        for (int i = 0; i < fields.length; ++i) {
            ...
            String dbFieldName = CamelCaseUtil.toUnderlineCase(fieldName);
    ...
    private String getKeysCondition(StringBuffer sql, OperateConstant mode) {
        ...
        if (OperateConstant.UPDATE.equals(mode))
            for (String pk : this.tableKeys)
                sql.append(" and " + pk + " = :" + CamelCaseUtil.toCamelCase(pk));
        else
            for (String pk : this.tableKeys)
                sql.append(" and " + pk + " = ?");
